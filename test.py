...
from datasets.aebad import AeBADDataset
...
def main():
    ...
    test_dataset = AeBADDataset(args.data_path, class_name=args.obj, is_train=False, resize=args.img_size)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=True, **kwargs)

    
    scores, test_imgs, recon_imgs, gt_list, gt_mask_list = test(args, model, test_loader, test_paras=False)
...
def test(args, model, test_loader,test_paras=False):
    model.eval()
    scores = []
    test_imgs = []
    gt_list = []
    ...
    if test_paras:
        data = torch.randn(1, args.input_channel, args.img_size, args.img_size).to(device)
        with torch.no_grad():
                data = data.to(device)
                for k in [1]:
                    ...
                    flops, params = profile(model, inputs=(inputs[0],), verbose=False)
        print('FLOPs: {:.2f} G, Params: {:.2f} M'.format(flops / 1e9, params / 1e6))
        raise NotImplementedError("Test parameters is not implemented yet.")
